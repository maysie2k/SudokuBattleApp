...
final class SoloBackgroundMusicService: NSObject, AVAudioPlayerDelegate {
    static let shared = SoloBackgroundMusicService()
    ...
    private func preparePlayersIfNeeded() {
        ...
        let names = ["Bittersweet Echoes", "Bittersweet Echoes-2"]
        for name in names {
            guard let url = Bundle.main.url(forResource: name, withExtension: "mp3", subdirectory: "Resources")
                ?? Bundle.main.url(forResource: name, withExtension: "mp3")
            else {
                print("[SoloBackgroundMusicService] Missing track: \(name).mp3")
                continue
            }
            ...
            do {
                ...
                print("[SoloBackgroundMusicService] Could not load \(name).mp3: \(error)")
            }
        }
